...
\date{2026-02-21~(\aidversion)}
\maketitle

\begin{abstract}
    The \AIDpackageName\ package implements and extension of the \emph{GAIDeT (Generative AI Delegation Taxonomy)}~\cite{Suchikova:2025:GAIDeT} to automate Generative AI disclosure statements and checklists.
...
The \AIDpackageName\ is designed to support emerging ethical, institutional, and publisher requirements concerning AI-assisted content creation.
It allows \LaTeX\ authors to:

\begin{itemize}
    \item Select specific tasks delegated to Generative~AI (AID) from an extension to the GAIDeT taxonomy~\cite{Suchikova:2025:GAIDeT} (e.g., idea generation, data cleaning, text summarization).
...
The package is available on CTAN at \url{https://ctan.org/pkg/aidisclose} and is included in TeX~Live and MiKTeX.
...
It provides an interactive generator for \AIDpackageName-based Generative AI disclosure statements, following the GAIDeT taxonomy~\cite{Suchikova:2025:GAIDeT}.
...
\subsection{Utilities and Internals}

The following commands are typically used internally but are available for advanced users who need to customize the package behavior or integrate it with other workflows.

\begin{description}
    \item[\latexinline{\AIDloadLanguage{<code>}}] \hfill \\
    Manually forces the loading of a specific language definition file, overriding the auto-detection.
    \begin{ltx}
\AIDloadLanguage{pt} % Force Portuguese strings
    \end{ltx}

    \item[\latexinline{\AIDpackageName}] \hfill \\
    Typesets the package name (\AIDpackageName). If \texttt{hyperref} is loaded, it links to the package's GitHub repository.

    \item[\latexinline{\AIDGetString{<key>}}] \hfill \\
    Returns the localized string associated with a specific internal key.
    \begin{ltx}
The title is: \AIDGetString{title_long}
    \end{ltx}

    \item[\latexinline{\aidversion}] \hfill \\
    A macro containing the current version number of the package (e.g., \texttt{\aidversion}).

    \item[\latexinline{\CiteLaTeX}, \latexinline{\CiteGAIDeT}] \hfill \\
    Macros that add citations to the \LaTeX{} project and the GAIDeT taxonomy, respectively.
    They are automatically handled based on the \texttt{nocite} option.
\end{description}
...
% Use <author>, <AUTHOR-LIST>, <declares>, <acknowledges> and <accept> as placeholders.
\AIDpreamble{<author> (<AUTHOR-LIST>) <declares> the use of AI\ldots}
\end{ltx}

\subsubsection{Unified Configuration Interface}
\label{sec:unified-config}

For convenience, the package provides two unified commands to handle configuration dynamically using key-value lists.

\paragraph{Package Options (\texttt{\textbackslash AIDset})}
The \latexinline|\AIDset| command allows you to change package load-time options dynamically in the preamble.

\begin{ltx}
\AIDset{
  autobib = true,  % Generate .bib file automatically
  nocite  = false  % Enable visible citations in the footnote
}
\end{ltx}

\paragraph{General Configuration (\texttt{\textbackslash AIDconfig})}
The \latexinline|\AIDconfig| command is a universal setter for content, visuals, and strings. It accepts the following keys:

\begin{description}
  \item[\texttt{tools}] Equivalent to \latexinline|\AIDtoolsUsed|.
  \item[\texttt{title}] Equivalent to \latexinline|\AIDdiscloseTitle|.
  \item[\texttt{order}] Sets the order of sections (see \latexinline|\AIDorder|).
  \item[\texttt{checkmark}] Sets the checkmark symbol.
  \item[\texttt{fontsize}] Sets the font size for the checklist.
  \item[\texttt{color\_used / color\_unused}] Sets text colors.
  \item[\texttt{color\_box\_used / color\_box\_unused}] Sets checkbox border colors.
  \item[\texttt{pre\_tools / post\_tools}] Text before/after the tools list.
  \item[\texttt{pre\_taxonomy / post\_taxonomy}] Text before/after the checklist.
  \item[\texttt{pre\_comments / post\_comments}] Text before/after the comments.
\end{description}
...
\begin{ltx}
\AIDrenderDeclaration[<options>]{<authors>}
\AIDrenderDeclaration*[<options>]{<authors>}
\end{ltx}

\begin{itemize}
    \item \textbf{Star variant (*):} Renders the checklist \emph{without} the section heading.
    \item \textbf{<options>:} Key-value options for rendering (default: \texttt{ncols=3}):
        \begin{description}
            \item[\texttt{ncols = <int>}] Number of columns for the checklist.
            \item[\texttt{lang  = <iso>}] Override the language for this declaration.
            \item[\texttt{label = <str>}] Assign a \LaTeX{} label to the section.
        \end{description}
...
The keys are derived from our extension to the GAIDeT taxonomy~\cite{Suchikova:2025:GAIDeT} and organized by research phase.
...
\subsection*{5. Data Management and Analysis (\texttt{d:*})}
\begin{gaikeys}
    \item[d:coll]  Data collection and scraping
    \item[d:val]   Data validation and quality check
    \item[d:cln]   Data cleaning and preprocessing
    \item[d:cur]   Data curation and organization
    \item[d:anl]   Data analysis
    \item[d:viz]   Visualization
    \item[d:rep]   Reproducibility testing
...
\begin{gaikeys}
    ...
    \item[sup:trd] Consistency checking against field trends
    \item[sup:lim] Identification of limitations
    \item[sup:rec] Publication strategy and journal selection
    \item[sup:pub] Publication support
\end{gaikeys}
...
\renewcommand\paragraph{\@startsection{paragraph}{4}{\z@}%
  ...
  {-1em}%                         % Space after (negative means run-in)
  {\normalfont\normalsize\bfseries\color{blue!80!black}}} % <--- Color added here!
\makeatother
...
Appendix~\ref{sec:aid_declaration} depicts a rendered declaration.
It was generated with the command
\begin{ltx}
\AIDrenderDeclaration[ncols=3]{Jane Doe, John Smith}
\end{ltx}
...
\AIDrenderDeclaration[ncols=3,label=sec:aid_declaration, lang=en]{Jane Doe, John Smith}
% \AIDrenderDeclaration[ncols=3]{Jane Doe}
