...
\texttt{\gaifilename} is a LaTeX package providing a standardized, transparent mechanism for declaring the use of Generative Artificial Intelligence (GAI) tools in academic, technical, and professional documents.
...
You can change the default title of the declaration section using \verb|\AIdiscloseTitle|.
\begin{verbatim}
\AIdiscloseTitle[Short Title]{Full Title for the Section}[section-level]
\end{verbatim}
...
To print the final declaration, use the \verb|\RenderAIDeclaration| command at the desired location in your document.  For papers/articles, you may consider to it before or after the references.  For thesis/dissertations, please follow your University/School rules.

\begin{verbatim}
\RenderAIDeclaration[<checklist_ncols>]{<Author Names>}
\end{verbatim}
...
\textbf{Example:}
\begin{verbatim}
\RenderAIDeclaration[1]{Jane Doe, John Smith}
\end{verbatim}
\noindent prints the declaration's checkist in a single column and considers the two given authors.

\section{Taxonomy Keys}
\label{sec:taxonomy-keys}
The following keys correspond to the specific tasks defined in the GAIDeT taxonomy~\cite{Suchikova:GAIDeT:2025}. Use these keys with the \verb|\Activate{}| command.
...
% \AIdiscloseTitle[Disclosure of Generative AI]
...
% Syntax: \RenderAIDeclaration[checklist_ncolumns]{Author1 Name, Author2 Name, …}
\RenderAIDeclaration[2]{Jane Doe, John Doe, Mary Doe}
% \RenderAIDeclaration[2]{Jane Doe}
